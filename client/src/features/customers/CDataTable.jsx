...
import { useGetCustomersQuery } from "../../store/api/customerApi.js";
import {
  getComparator,
  sortedFilteredData,
  filterData,
...
const CDataTable = () => {
  ...
  const filteredData = customers.map((item) => {
    return {
      ...
      cId: item.id,
      ordersCount: item.orders?.length ? item.orders.length : 0,
        ...
        >
          <TableHead>
            <TableRow>
              ...
              <TableCell align="left" key="ordersCount">
                <TableSortLabel
                  active={valueToOrderBy === "ordersCount"}
                  direction={
                    valueToOrderBy === "ordersCount" ? orderDirection : "asc"
                  }
                  onClick={createSortHandler("ordersCount")}
                >
                  Sipariş Sayısı
                </TableSortLabel>
              </TableCell>
